...
@var array grid The map. Each 'cell' contains its type.
*/
var MapGen=function(x, y, passes, cleanup){
  // base cell types
  const WALL='wall';
  const FLOOR='floor';
 ...
 // Returns the average type of surrounding cells.
 this._getCellMoreDirectNeighboursType=function (x,y){
  ...
  @var int cleanup  Whether or not clean the map (remove lonely cells)
                    Value from 0 to 5, 0 being no cleanup and 5 the max.
  */
  this._createMap=function(x,y, passes, cleanup){
    var wallPercent=50; // Percentage of walls
    var percentage= 80; // When creating sub-cells, percentage of chances to have the same type:
    // First pass:
    this.grid=[];
    var walls=0;
    var floors=0;
    ...
    console.log({GRID_GENERATED:true, wallPercent:wallPercent, walls:walls, floors:floors})
    ...
    for(let p=0; p<passes; p++){
      ...
      for(let i=0; i<this.grid.length; i++){
        ...
        for(let j=0; j<this.grid[i].length; j++){
          ...
          newRow1.push((Math.random()*100<percentage)?type:inverse);
          newRow1.push((Math.random()*100<percentage)?type:inverse);
          newRow2.push((Math.random()*100<percentage)?type:inverse);
          newRow2.push((Math.random()*100<percentage)?type:inverse);
        }
        newMap.push(newRow1);
        newMap.push(newRow2);
    ...
    if(cleanup>0){
      for(let i=0; i<this.grid.length; i++){
        for(let j=0; j<this.grid[i].length; j++){
          ...
          if(this._getCellSameDirectNeighbours(i, j)<=cleanup-1){
  ...
  this.jQueryRender=function(target, prefix){
    ...
    for(let i=0; i<this.grid.length; i++){
      ...
      for(let j=0; j<this.grid.length; j++){
        $('#'+prefix+'row'+i).append('<div id="'+prefix+'cell-'+i+'-'+j+'" class="cell cell-'+this.grid[i][j]+' room-'+this.roomsData[i+':'+j]+'"></div>');
      }
    }
  }

  // Creates the base map.
  //this._createMap(x, y, passes, cleanup);
}
...
var map=new MapGen;
var map2=new MapGen;
map._createMap(5, 5, 4, 2);
/*
map2.grid=map.grid;
map2._findRooms();
map2.jQueryRender('#grid2', 'g2');
*/
map._findRooms();
map._cleanRooms(50);
map._locateRooms();
