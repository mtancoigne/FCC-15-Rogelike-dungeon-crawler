...
class App extends React.Component{
  constructor(props){
    ...
    // Generate the map
    var map=new MapGen();
    //x, y, passes, cleanLevel, wallPercent, sameSubCellPercent
    map.createMap(this.props.options);
    map.createRooms();
  ...
  render(){
    var i=0;
    var rows=this.state.map.grid.map((row)=>{
      return (<Row cells={row} key={i} rowId={i} />);
      i++
    })
    return(
      <div>
...
var options=;
$(document).ready(()=>{
  render(<App options={options}/>, document.getElementById('grid'));
})
